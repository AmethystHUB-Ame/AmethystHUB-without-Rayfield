...
--[[
    ...
    ─────────────────────────────────────────────────────────────
    BUGS: Fixed 'or true' input bug, Auto Strafe speed, teleport retry,
          Fast Respawn leak, Fullbright/LightingUltra conflict, duplicate
          refreshVP, GodMode event-driven, Silent Aim _G exposure
    PERF: Chunked OmegaFPS, shared visibility cache, removed WalkSpeed polling
    ARCH: ToggleRegistry, _S_categories for save/load, deferred loadConfig
    SEC:  Randomized GUI names, local Config, AimHumanization 2.0, KillAura rate limit
    QUAL: _alive flag, trackFeatureConn, notification queue cap, tick→os.clock
]]
...
local _alive = true -- [V4.1] Cancellable loop flag
...
-- [V4.1] Feature connection tracking for reliable cleanup
local _featureConns = {}
...
-- [V4.1] Named constants (replace magic numbers)
local CONST = {
    OMEGA_FPS_CHUNK_SIZE = 500,
    VISIBILITY_CACHE_TTL = 0.15,
    NOTIFICATION_MAX_QUEUE = 10,
    KILLAURA_COOLDOWN = 0.33,
    MAX_TELEPORT_RETRIES = 3,
    STRAFE_SPEED_MULT = 0.7,
    AIM_HUMANIZATION = 2.0,
    SERVER_FETCH_TIMEOUT = 30,
...
local Config = { -- [V4.1] Local scope, not global
    Version            = "4.1.0",
    ...
    AimHumanization    = 0.8,        -- [V4.1] Smooth drift range in studs (uses persistent noise, not per-frame random)
    AutoHopInterval    = 15,
    AntiAFKInterval    = 55,
    CacheBuildDelay    = 0.5,
}
...
-- [V4.1] Auto-save config periodically
task.spawn(function()
    while _alive do
...
local function cleanupAll()
    _alive = false -- [V4.1] Signal all loops to stop
    -- [V4.1] Disconnect tracked feature connections
    ...
    _safeCall(function()
        for _, gui in ipairs(CoreGui:GetChildren()) do
            if gui:GetAttribute("__AmethystV4") -- [V4.1] Detect by attribute, not name
               or gui.Name == "AmethystToggleV4" or gui.Name == "AmethystFlyButtons"
               or gui.Name == "AmethystHubV4" or gui.Name == "AmethystWatermarkV4" then
                gui:Destroy()
            end
        end
...
-- [V4.1] Flat state — separate category tables for save/load, merged into S for fast access
local S_Combat = {
    Aimbot        = false,
    AimbotSmooth  = 3,
    AimPart       = "Head",
...
-- [V4.1] Flat merged table — zero metatable overhead on hot paths
local S = {}
...
-- [V4.1] Toggle Registry — config↔UI synchronization
local ToggleRegistry = {}

-- Write-through metatable: keeps category tables in sync for save/load
setmetatable(S, {
...
-- [V4.1] loadConfig deferred to after UI build for toggle sync
-- loadConfig()  -- called later after UI is built
-- ============================================================
...
-- [V4.1] Shared visibility cache — halves raycast cost
local _visCache = {}
local _visCacheTime = {}

local function isPlayerVisible(player)
    local now = os.clock()
    if _visCacheTime[player] and (now - _visCacheTime[player]) < CONST.VISIBILITY_CACHE_TTL then
        return _visCache[player]
    end
    local cam = workspace.CurrentCamera
    local pc = PartCache and PartCache[player]
    if not pc or not pc.head or not cam then
        _visCache[player] = false
        _visCacheTime[player] = now
        return false
    end
    local origin = cam.CFrame.Position
    local dir = (pc.head.Position - origin)
    local ray = workspace:Raycast(origin, dir, RaycastParams.new())
    local visible = (ray == nil) or (ray.Instance and ray.Instance:IsDescendantOf(player.Character))
    _visCache[player] = visible
    _visCacheTime[player] = now
    return visible
end
...
function runOmegaFPS()
    ...
    for i, d in ipairs(descendants) do
        if i % CONST.OMEGA_FPS_CHUNK_SIZE == 0 then task.wait() end -- [V4.1] Yield every chunk
        if S.FPSBoost then
            _safeCall(function()
                simplifyPart(d)
                count = count + 1
...
function applyLightingUltra()
    requestLighting("LightingUltra") -- [V4.1]
    _safeCall(function()
...
function applyFullbright(on)
    _safeCall(function()
        if on then
            requestLighting("Fullbright") -- [V4.1]
...
function hideESP(esp)
    ...
    if esp.highlight then esp.highlight.Enabled = S.Wallhack end -- [V4.1 FIX] Keep highlight if Wallhack is on
end
...
cachedTarget = nil -- [V4.1] No _G exposure
...
function getClosestEnemy(now)
    ...
    cachedTarget = best; cachedTarget = best -- [V4.1] No _G exposure
    return best
end
...
function fetchSmallestServers(placeId, maxPages)
    ...
    serverFetching = true
    local _fetchStart = os.clock() -- [V4.1] Timeout protection
    local all = {}
    local cursor = ""
    local page = 0
...
function enableGodMode()
    -- [V4.1] Event-driven: only heal when damaged, not every frame
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
...
-- ============================================================
local kaThrottle = 0
kaActivateCooldown = 0 -- [V4.1] Rate limiter
                ...
                if (pc.root.Position - root.Position).Magnitude <= S.KillAuraRange then
                    _safeCall(function()
                        ...
                        if aim then
                            ...
                            if now - kaActivateCooldown >= CONST.KILLAURA_COOLDOWN then -- [V4.1]
...
function updateESP(now)
    refreshVP()
    local anyESP = S.ESP_Box or S.ESP_Skeleton or S.ESP_Tracer or S.ESP_Distance or S.ESP_Name or S.ESP_Health or S.Wallhack -- [V4.1 FIX] Include Wallhack so highlights aren't killed
    if not anyESP then
    ...
    for player, esp in pairs(ESPPool) do
        ...
        if not pc then hideESP(esp)
            ...
            if not hum or hum.Health <= 0 or not head or not root then hideESP(esp)
                ...
                -- [V4.1 FIX] Wallhack highlight is INDEPENDENT of Drawing ESP and VisCheck
                -- Always sync highlight state to S.Wallhack, regardless of visibility
                if esp.highlight then
                    esp.highlight.Enabled = S.Wallhack
                end
            end
        end
    end
...
createScreenGui = function(name, displayOrder)
    ...
    gui.Name = "SG_" .. HttpService:GenerateGUID(false) -- [V4.1] Randomized name
    gui:SetAttribute("__AmethystV4", true) -- [V4.1] Cleanup marker
...
local notifQueue = {}
local NOTIFICATION_MAX_QUEUE = CONST.NOTIFICATION_MAX_QUEUE -- [V4.1]
...
local function showNotification(title, content, duration, severity)
    ...
    if visible < MAX_VISIBLE_NOTIFS then
        ...
        -- [V4.1] Cap notification queue
        if #notifQueue >= NOTIFICATION_MAX_QUEUE then
...
createInput = function(parent, config)
    ...
    track(inputBox.FocusLost:Connect(function(enterPressed)
        ...
        if enterPressed then -- [V4.1 FIX] removed "or true" that fired on every unfocus
...
-- [V4.1] Now that UI is built, load config and sync toggles
loadConfig()
for key, api in pairs(ToggleRegistry) do
    if api.set then
        _safeCall(function() api.set(S[key]) end)
    end
end
...
-- [V4.1] Shared hub visibility toggle (eliminates duplication)
...
local _activeInput = nil -- [V4.1 FIX] Track the SPECIFIC input object that started on the button
...
track(toggleBtn.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1
		...
		dragging = true
		_activeInput = input -- [V4.1 FIX] Remember which finger/click started on button
		dragStart = input.Position
...
track(UserInputService.InputChanged:Connect(function(input)
	if not dragging or input ~= _activeInput then return end -- [V4.1 FIX] Only track the finger that started on button
...
track(UserInputService.InputEnded:Connect(function(input)
	if input ~= _activeInput then return end -- [V4.1 FIX] Only respond to the SAME finger that started on button
	if input.UserInputType == Enum.UserInputType.MouseButton1
		or input.UserInputType == Enum.UserInputType.Touch then
		if dragging then
			dragging = false
			_activeInput = nil -- [V4.1 FIX] Clear tracked input
...
-- [V4.1 FIX] Activated fallback — Roblox fires this for clean taps even during multi-touch combat
-- This catches taps that the InputBegan/InputEnded system may miss
track(toggleBtn.Activated:Connect(function()
	if totalDragDist <= 10 and not dragging then
		toggleHubVisibility()
	end
end))
...
local function onCharAdded(player, char)
    ...
    if player == LocalPlayer then
        ...
        -- [V4.1] ChildRemoved: clean up weaponCache when tools are removed
        ...
        if hum and S.FastRespawn then
            local _frConn -- [V4.1] Store to prevent leak
            _frConn = hum.Died:Connect(function()
                if _frConn then _frConn:Disconnect(); _frConn = nil end -- [V4.1] One-shot
...
-- [V4.1] Removed duplicate refreshVP (see first definition above)
...
track(RunService.Heartbeat:Connect(function(dt)
    ...
    if S.AutoStrafe then _safeCall(function()
        ...
        if hum and root and hum.MoveDirection.Magnitude > 0 then
            ...
            local strafeSpeed = (S.Speed and S.SpeedVal or 16) * CONST.STRAFE_SPEED_MULT -- [V4.1] Use base speed when Speed off
...
-- [V4.1 FIX] Smooth persistent aim noise (replaces violent per-frame random)
-- Instead of new random every frame, maintain a drift offset that slowly wanders
local _aimNoiseOffset = Vector3.new(0, 0, 0) -- current smooth noise position
local _aimNoiseTarget = Vector3.new(0, 0, 0) -- target position to drift toward
local _aimNoiseTimer = 0 -- time until next target change
track(RunService.RenderStepped:Connect(function(delta)
    ...
    -- [V4.1 FIX] Aimbot FIRST — must modify Camera.CFrame BEFORE ESP reads it
    -- (Previously ESP ran first, used pre-aimbot camera → frozen ESP inside FOV)
    if S.Aimbot then
        ...
        if target then
            ...
            if pc then
                ...
                if aim then
                    ...
                    -- [V4.1 FIX] Smooth persistent aim humanization (replaces per-frame random jitter)
                    -- Drift offset slowly wanders toward a new random target every 0.3-0.7s
                    -- This looks human (slightly off-center) without violent frame-to-frame shaking
                    local hRange = Config.AimHumanization * 0.5
                    _aimNoiseTimer = _aimNoiseTimer - delta
                    if _aimNoiseTimer <= 0 then
                        _aimNoiseTimer = 0.3 + math.random() * 0.4 -- new target every 0.3-0.7s
                        _aimNoiseTarget = Vector3.new(
                            (math.random() - 0.5) * hRange,
                            (math.random() - 0.5) * hRange * 0.6, -- less vertical drift
                            (math.random() - 0.5) * hRange
                        )
                    end
                    -- Smoothly interpolate current noise toward target (no sudden jumps)
                    local noiseLerp = math.clamp(delta * 5, 0, 1)
                    _aimNoiseOffset = _aimNoiseOffset:Lerp(_aimNoiseTarget, noiseLerp)
                    aimPos = aimPos + _aimNoiseOffset
    ...
    -- [V4.1 FIX] Runs AFTER aimbot so WorldToViewportPoint uses final camera position
